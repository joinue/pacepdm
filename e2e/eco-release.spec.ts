  ...
  test("create an ECO and attempt to submit for review", async ({ page }) => {
    ...
    await expect(async () => {
      const successCopy = page.getByText(
        /submitted for approval|status changed to submitted/i
      );
      const errorCopy = page.getByText(
        /no.*workflow|failed|error|workflow is required/i
      );
      const seen =
        (await successCopy.first().isVisible().catch(() => false)) ||
        (await errorCopy.first().isVisible().catch(() => false));
