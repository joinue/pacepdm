...
- **BOM-2, LOCKED cost source — done.** Part edits are refused only when the cost would actually change (`unitCostWouldChange`), the form stops sending cost under the lock, create refuses a cost, and the CSV importer skips cost and says so. The QuickBooks importer still writes cost under the lock, on the reading that QuickBooks is the system the lock defers to; the owner deferred that question on 2026-09-15, so leave it until they raise it.
- **BOM-7, AUTO part numbering — done.** On a collision the next number is moved past the highest existing number the prefix and padding could produce (trash included — trashed parts keep their number under the unique index), using PostgREST's regex `match` operator, with the same compare-and-swap. Manual numbers and CSV imports advance the counter. Not yet checked against production PostgREST; if `match` is refused, creation falls back to stepping one number at a time.
...
- **UI-3, record links — done.** Links point at `/ecos/[id]` and `/boms/[id]`; the list pages redirect old `?ecoId=` / `?bomId=` links. The vault accepts `?file=` — which existing notification rows store — as well as `?fileId=`, and follows query-string changes it did not make. ECO notifications (created, status change, implemented) link to the ECO too, since 2026-09-15.
