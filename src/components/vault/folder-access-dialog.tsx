  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setRoles(
          (r || []).map((x) => ({ id: x.id, label: x.name, sublabel: x.description ?? undefined }))
        );
      } catch {
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Folders are public by default. Adding any ALLOW rule restricts the folder to the
              principals you list — everyone else loses access. DENY rows override ALLOW.
            </DialogDescription>
          </DialogHeader>
          ...
          {!loading && (
            <div
              className={
                rows.length === 0
                  ? "flex items-start gap-2 rounded-md border border-info/30 bg-info/10 /50 /30 p-3 text-xs text-info"
                  : "flex items-start gap-2 rounded-md border border-warning/30 bg-warning/10 /50 /30 p-3 text-xs text-warning"
              }
            >
              {rows.length === 0 ? (
                <>
                  ...
                  <div>
                    <strong className="font-semibold">No direct rules.</strong> This folder is
                    public unless a parent folder restricts it. Adding an ALLOW rule below will lock
                    it down to the principals you list.
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div>
                    <strong className="font-semibold">Restricted.</strong> Only the principals
                    listed below have access here — every other workspace member is excluded.
                  </div>
                </>
              )}
            </div>
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto -mx-1 px-1">
            ...
            ) : rows.length === 0 ? null : ( // need for a redundant message here. // Banner above already covers the "no direct rules" state — no
              rows.map((row) => (
                <div key={row.id} className="flex items-start gap-2 p-2.5 border rounded-md">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Select
                  value={principalType}
                  onValueChange={(v) => {
                    setPrincipalType(v as "USER" | "ROLE");
                    setPrincipalId("");
                  }}
                >
                  ...
                  <SelectContent>
                    {principalOptions.map((o) => (
                      <SelectItem key={o.id} value={o.id}>
                        {o.label}
                        {o.sublabel && (
                          <span className="text-xs text-muted-foreground ml-2">{o.sublabel}</span>
                        )}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={level}
                  onValueChange={(v) => setLevel(v as "VIEW" | "EDIT" | "ADMIN")}
                >
                  <SelectTrigger className="w-full">
                    <SelectValue>
                      {(v) =>
                        (
                          ({ VIEW: "View", EDIT: "Edit", ADMIN: "Admin" }) as Record<string, string>
                        )[v as string] ?? ""
                      }
                    </SelectValue>
                  </SelectTrigger>
      ...
      <AlertDialog open={confirmFirstRule} onOpenChange={setConfirmFirstRule}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This is the first direct rule on <strong>{folderName}</strong>. Once added, only
              principals matching an ALLOW row (here or inherited from a parent) will be able to see
              this folder. Everyone else in the workspace will lose access until you grant it
              explicitly.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
