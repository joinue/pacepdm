...
export function VaultDialogs({ vault }: VaultDialogsProps) {
  return (
    <>
      {/* Rename Dialog */}
      <Dialog
        open={!!vault.renameTarget}
        onOpenChange={(open) => !open && vault.setRenameTarget(null)}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Rename {vault.renameTarget?.type}</DialogTitle>
            <DialogDescription>
              Enter a new name for &ldquo;{vault.renameTarget?.name}&rdquo;
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            ...
            <Input
              id="rename"
              value={vault.newName}
              onChange={(e) => vault.setNewName(e.target.value)}
              className="mt-2"
              onKeyDown={(e) => e.key === "Enter" && vault.handleRename()}
            />
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => vault.setRenameTarget(null)}>
              Cancel
            </Button>
            <Button
              onClick={vault.handleRename}
              disabled={!vault.newName.trim() || vault.newName === vault.renameTarget?.name}
            >
              Rename
            </Button>
          </DialogFooter>
      ...
      {/* Delete Confirm */}
      <AlertDialog
        open={!!vault.deleteTarget}
        onOpenChange={(open) => !open && vault.setDeleteTarget(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
      ...
      {/* Transition Dialog */}
      <Dialog
        open={!!vault.transitionTarget}
        onOpenChange={(open) => !open && vault.setTransitionTarget(null)}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Change Lifecycle State</DialogTitle>
            <DialogDescription>
              Select a transition for &ldquo;{vault.transitionTarget?.fileName}&rdquo;
            </DialogDescription>
          </DialogHeader>
            ...
            {vault.transitions.length === 0 ? (
              ...
              vault.transitions.map((t) => (
                <Button
                  ...
                  onClick={() => vault.handleTransition(t.id)}
                >
      ...
      <Dialog open={!!vault.moveTarget} onOpenChange={(open) => !open && vault.setMoveTarget(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Select a destination folder for &ldquo;{vault.moveTarget?.name}&rdquo;
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <Label>Destination folder</Label>
            <Select
              value={vault.moveDestination}
              onValueChange={(v) => vault.setMoveDestination(v ?? "")}
            >
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => vault.setMoveTarget(null)}>
              Cancel
            </Button>
            <Button onClick={vault.handleMove} disabled={!vault.moveDestination}>
              Move
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <AlertDialog open={vault.showBulkDeleteConfirm} onOpenChange={vault.setShowBulkDeleteConfirm}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={vault.handleBulkDelete}
              className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
            >
              Delete All
            </AlertDialogAction>
