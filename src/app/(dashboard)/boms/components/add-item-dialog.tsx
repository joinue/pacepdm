...
import { Plus, Search, Loader2, X, FileText, Cpu, Link2, Package } from "lucide-react";
                    ...
                    <div className="border rounded-lg max-h-40 overflow-y-auto">
                      {boms.filter((b) => b.id !== selectedBomId).length === 0 ? (
                        <p className="text-xs text-muted-foreground text-center py-3">
                          No other BOMs available
                        </p>
                      ) : (
                        boms
                          .filter((b) => b.id !== selectedBomId)
                          .map((b) => (
                            <button
                              key={b.id}
                              type="button"
                            ...
                            >
                              ...
                              <StatusBadge
                                status={b.status}
                                kind="bom"
                                className="text-4xs px-1 py-0 ml-auto shrink-0"
                              />
                            </button>
                          ))
                      )}
