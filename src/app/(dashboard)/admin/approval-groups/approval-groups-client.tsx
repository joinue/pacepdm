  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        description="Manage groups of users. Attach groups to approval steps in Admin → Workflows."
        actions={
          <>
      ...
      {error ? (
        ...
        <Card>
          <CardContent className="py-8 text-center text-muted-foreground">
            ...
            <p className="text-sm">
              No active approval groups. Create one to set up approval workflows.
            </p>
          </CardContent>
        </Card>
      ) : (
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Create a group of users who can approve transitions.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="What does this group approve?"
                  rows={2}
                />
              </div>
            </div>
      ...
      <Dialog open={!!addMemberGroup} onOpenChange={(open) => !open && setAddMemberGroup(null)}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setAddMemberGroup(null)}>
              Cancel
            </Button>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {willArchive ? (
                <>
                  &quot;{deleteTarget?.name}&quot; is referenced by {deleteTarget?.usageCount}{" "}
                  workflow step
                  {deleteTarget?.usageCount === 1 ? "" : "s"}, so it will be archived instead of
                  deleted to preserve the audit trail. It will no longer appear in workflow pickers
                  and can be restored later from the archived section.
                </>
              ) : (
                <>
                  This group has never been used. It will be permanently deleted along with its
                  members.
                </>
              )}
            </AlertDialogDescription>
